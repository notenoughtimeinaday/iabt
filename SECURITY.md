...
Project and AI-usage records use owner-scoped row-level security. Account entitlements are readable by their owner and managed through administrator or verified backend workflows. The AI generation function requires an authenticated Base44 user and enforces per-user hourly safety limits plus monthly allowances. Paid plans can apply included credits to eligible production; purchased credits are stored on the administrator-managed entitlement for Free-plan production and paid-plan overage.
...
- Paid supplier work requires enough eligible IABT credits, a documented approved ProviderAgreement, a commercial-approval secret, the minimum margin floor, per-job limits, and daily/monthly spend capacity. Free-plan paid production requires purchased credits; paid plans use included credits first.

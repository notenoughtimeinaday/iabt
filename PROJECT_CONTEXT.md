...
- Customers buy IABT plans and credits from Insured Spending, LLC through the platform Stripe account. A generated project's own commerce must use the project owner's separately authorized Stripe account; the platform Stripe secret is never copied into generated code. External generation defaults to the user's selected provider-account route when a reviewed adapter is available, with optional IABT-managed production remaining behind exact quotes, supplier controls, and approval gates.

...
function escapeHtml(value) {
  return clean(value, 500).replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;");
}
function titleFor(requestText, spec) {
...
function createFallbackAppDefinition(requestText, spec = {}) {
  ...
  const featureList = Array.isArray(spec?.features) ? spec.features.map((item) => clean(item, 120)).filter(Boolean).slice(0, 6) : [];
  return {
...
function refresh(){status.textContent="Octave "+octave+" \xB7 "+(context&&context.state==="running"?"Audio ready":"Audio off")}
...
function genericHtml(title, requestText) {

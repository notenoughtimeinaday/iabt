// Standalone Exchange policy. No platform runtime or network dependencies.
...
export function text(value, max = 500, fallback = "") {
    const result = String(value ?? fallback).trim();
    return result.slice(0, max);
}
export function list(value, max = 30, itemMax = 120) {
...
export function enumList(value, allowed, max = 20) {
...
export function sanitizeProfileInput(body, user, existing = null) {
    const createdAt = existing?.created_at || nowIso();
    const profile = {
        user_id: String(user.id),
        user_email: String(user.email),
        public_alias: text(body?.public_alias, 80, existing?.public_alias || user.name || user.full_name || "IABT member"),
    ...
    if (!profile.public_alias)
        throw new Error("A public alias is required.");
    if (!profile.headline)
        throw new Error("A collaboration headline is required.");
    if (profile.status === "active" && profile.skills.length === 0) {
        throw new Error("Add at least one skill before activating your profile.");
    }
    return profile;
}
export function sanitizeNeedInput(body, user, existing = null) {
    const createdAt = existing?.created_at || nowIso();
    const need = {
    ...
    if (!need.title)
        throw new Error("A need title is required.");
    if (!need.public_summary)
        throw new Error("A public project summary is required.");
...
function phraseCoverage(required, offered) {
    if (!required.length)
        return 1;
    ...
    for (const raw of required) {
        ...
        if (!wanted)
            continue;
        const direct = offeredNormalized.some((candidate) => candidate === wanted || candidate.includes(wanted) || wanted.includes(candidate));
        if (direct) {
            matched += 1;
            continue;
        }
        const wantedTokens = wanted.split(" ").filter((item) => item.length > 2);
        if (wantedTokens.length && wantedTokens.every((item) => offeredTokens.has(item)))
            matched += 1;
    }
    return Math.max(0, Math.min(1, matched / required.length));
}
function intersectionCoverage(required, offered) {
    if (!required.length)
        return 1;
    if (!offered.length)
        return 0;
...
function openCompatible(required, offered) {
    if (!required.length)
        return true;
    if (!offered.length)
        return false;
    if (required.includes("open_to_discussion") || offered.includes("open_to_discussion"))
        return true;
    return intersectionCoverage(required, offered) > 0;
}
function credentialCoverage(required, claims) {
    if (!required.length)
        return 1;
    const verified = (claims || [])
        .filter((claim) => claim?.verification_status === "verified" && (!claim.expires_at || new Date(claim.expires_at).getTime() > Date.now()))
        // Mandatory credentials are reviewed types, never free-text mentions.
        // A claim saying "not a licensed engineer" must not satisfy that gate.
    ...
    for (const item of required) {
        ...
        if (verified.some((candidate) => candidate === wanted))
            matched += 1;
    }
    return matched / required.length;
}
function availabilityScore(value) {
    return { full_time: 1, project_based: 0.9, part_time: 0.8, limited: 0.45, unavailable: 0 }[value] ?? 0.6;
}
function reputationScore(profile, claims) {
    const verifiedClaims = (claims || []).filter((claim) => claim?.verification_status === "verified" && (!claim.expires_at || new Date(claim.expires_at).getTime() > Date.now())).length;
    const level = {
        unverified: 0.2,
        identity_verified: 0.55,
        credential_verified: 0.85,
        organization_verified: 1,
    }[profile?.verification_level] ?? 0.2;
    return Math.min(1, level + Math.min(0.2, verifiedClaims * 0.05));
}
export function evaluateCandidate(need, profile, claims = []) {
    const hardFilters = {
    ...
    const strengths = [];
    if (breakdown.capability_fit >= 25)
        strengths.push("strong capability overlap");
    if (breakdown.jurisdiction_credentials >= 14)
        strengths.push("jurisdiction or credential alignment");
    if (breakdown.availability >= 11)
        strengths.push("compatible availability");
    if (breakdown.project_stage >= 11)
        strengths.push("project-stage fit");
    if (breakdown.relationship_compensation >= 7)
        strengths.push("compatible working relationship");
    if (breakdown.reputation >= 4)
        strengths.push("verified reputation signals");
    return {
        eligible,
...
export function profileCard(profile, claims = []) {
    return {
...
export function disclosedContact(profile, user, fields) {
    const allowed = new Set(enumList(fields, DISCLOSURE_FIELDS, 10));
    const result = {};
    if (allowed.has("full_name"))
        result.full_name = text(user?.full_name || user?.name, 160);
    if (allowed.has("email"))
        result.email = text(user?.email || profile?.user_email, 240);
    if (allowed.has("phone"))
        result.phone = text(profile?.phone, 60);
    if (allowed.has("organization"))
        result.organization = text(profile?.organization, 160);
    if (allowed.has("calendar_link"))
        result.calendar_link = cleanUrl(profile?.calendar_link);
    if (allowed.has("linkedin"))
        result.linkedin = cleanUrl(profile?.linkedin);
    if (allowed.has("website"))
        result.website = cleanUrl(profile?.website);

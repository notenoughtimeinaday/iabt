...
Deno.serve(async (req) => {
  ...
  try {
    ...
    artifact = await service.entities.CreationArtifact.create({
      ...
      name: job.quote_snapshot?.owner_demo_only ? "IABT owner demo video.mp4" : "IABT video.mp4",
      kind: "video",
      ...
      metadata: {
        ...
        owner_demo_only: job.quote_snapshot?.owner_demo_only === true,
        commercial_release_approved: job.quote_snapshot?.commercial_release_approved === true,
